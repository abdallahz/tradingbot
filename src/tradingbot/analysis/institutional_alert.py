...
def format_institutional_alert(
    ...
    lines = [
        ...
        f"Stop Loss : <code>${ctx.recommended_stop:.2f}</code>  (structure-based)",
        f"TP 1      : <code>${ctx.tp1_conservative:.2f}</code>  ({ctx.risk_reward_ratio:.1f}R)",
        f"TP 2      : <code>${ctx.tp2_aggressive:.2f}</code>  (extension)",
        "",
        f"Risk/Share: <code>${ctx.risk_per_share:.2f}</code>",
        f"R:R Ratio : <code>{ctx.risk_reward_ratio:.1f}:1</code>",
