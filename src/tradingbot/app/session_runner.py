...
from tradingbot.utils.timezone import ET as _ET_TZ, now_et as _now_et_fn
...
from tradingbot.strategy.night_research_selector import NightResearchSelector
...
from tradingbot.analysis.confluence_engine import evaluate_confluence
...
from tradingbot.web.alert_store import card_to_dict, save_alert, get_today_alerted_symbols, get_stopped_symbols_data, get_today_alert_counts
...
class SessionRunner:
    # Class-level defaults — overridden by __init__ from config YAML
    _catalyst_min_night_score: int = 40
    _catalyst_bypass_score: int = 70
    _catalyst_relaxed_gate: int = 55
    _opening_window_confluence_floor: int = 15
    _ranker_weight: float = 0.60
    _confluence_weight: float = 0.40
    # Lazily-initialized helpers — overridden by __init__ with config values
    _card_builder: "CardBuilder" = None  # type: ignore[assignment]
    _tuning_overrides: dict = {}
    _market_condition = None
    _market_health = None
    ...
    def __init__(self, root: Path, use_real_data: bool = False, skip_market_data: bool = False) -> None:
        ...
        catalyst_cfg = scanner_config.get("catalyst", {})
        scoring_cfg = scanner_config.get("scoring", {})
        self._catalyst_min_night_score: int = catalyst_cfg.get("min_night_score", 40)
        self._catalyst_bypass_score: int = catalyst_cfg.get("bypass_score", 70)
        self._catalyst_relaxed_gate: int = catalyst_cfg.get("relaxed_gate_score", 55)
        self._opening_window_confluence_floor: int = scoring_cfg.get("opening_window_confluence_floor", 15)
        self._ranker_weight: float = scoring_cfg.get("ranker_weight", 0.60)
        self._confluence_weight: float = scoring_cfg.get("confluence_weight", 0.40)
        ...
        self.relaxed_ranker = CatalystWeightedRanker(
            min_score=scoring_cfg.get("o2_min_score", 25),
            max_candidates=scanner_defaults["max_candidates"],
        )
        ...
        self._night_selector = NightResearchSelector(
            min_catalyst_score=self._catalyst_min_night_score,
    ...
    def run_day(self) -> tuple[WatchlistRun, WatchlistRun]:
        ...
        if self.use_real_data and self.data_client and self.catalyst_scorer:
            ...
            night_universe_str = [s for s, score in catalyst_scores.items() if score >= self._catalyst_min_night_score]
    ...
    def _build_cards(
        ...
        # Load stop-out data (HOD + stop price) for re-entry evaluation,
        # and today's alert counts for the per-symbol re-entry cap.
        stopped_data = get_stopped_symbols_data()
        alert_counts = get_today_alert_counts()
        ...
        for item in ranked:
            ...
            if not skip_dedup and not cb.passes_dedup(symbol, already_alerted, dropped, stopped_data=stopped_data, alert_counts=alert_counts):
                continue
            ...
            if not cb.passes_vwap_distance(symbol, dropped, session_tag, market_condition=self._market_condition, tuning_overrides=self._tuning_overrides):
                continue
            ...
            if not cb.passes_intraday_extension(symbol, dropped, tuning_overrides=self._tuning_overrides):
                continue
            ...
            card.score = round(min(100.0,
                card.score * self._ranker_weight + confluence_result.composite_score * self._confluence_weight
            ), 2)
    ...
    def _get_night_research_picks(
        ...
        return self._night_selector.build_picks(snapshots, catalyst_scores, session_tag)
    ...
    def run_single_session(
        ...
        *,
        ...
        # Build universe: prefer symbols above min_night_score threshold.
        # Fall back to top-50 by score so we always have something to scan.
        universe_str = [s for s, sc in catalyst_scores.items() if sc >= self._catalyst_min_night_score]
        if not universe_str:
